...
%%
%My Files
% file_01  = '../OUT_files/ocean_his_0001.nc';
% file_river = '../In_files/rivers_tracer.nc';

%Julias
file_01  = 'ocean_his_0001.nc';
file_river = 'rivers_tracer.nc';

fn       = [file_01];

% Initial conditions
salt_0 = ncread(fn, 'salt', [1,1,30,1], [inf, inf, 1, 1]);
...
% bird view
% subplot(131)
salt = ncread(fn,'salt',[1 1 1 t_idx_e],[inf inf inf 1]); 
pcolor(xr./1000, yr./1000, salt(:,:,30))
...
title(['Day = ', num2str(oceantm(t_idx_e)./86400)])
%%
subplot(132)
salt = ncread(fn,'salt',[1 1 1 1],[inf inf inf 1]); 
pcolor(xr./1000, yr./1000, salt(:,:,30))
colorbar, ca = max(abs(caxis));
% colormap(gca, flipud(redblu))
% caxis([-ca ca])
...
subplot(133)
salt = ncread(fn,'salt',[1 1 1 10],[inf inf inf 1]); 
pcolor(xr./1000, yr./1000, salt(:,:,30))
colorbar, ca = max(abs(caxis));
% colormap(gca, flipud(redblu))
% caxis([-ca ca])
