...
% asr Sept2019 modified for bplume experiment
...
%    init_file='joe_tc_ocean_init.nc';
     init_file='tracer_init_ale_slope2_C2.nc';
...
%     data_file='coawst_us_20121024_13.nc';
    ...
    % This are added by SNC
    s_river = 0;       % enter river salinity by SNC
...
%4) Set values of theta_s, theta_b, Tcline, and N from your *.in file.
    ...
    else  % Modified to match .in file
      theta_s = 5.0;
      theta_b = 0.4;
      Tcline =  5.0;
      Vtransform = 2;
...
%     and river info
   grid_file  = 'grid_case2_finergrid03_slope2_ale.nc' ;  %<-enter name of grid here
...
%
  ...
  if (read_init)
    ...
    salt(1:xi_rho,1:eta_rho,1:N,1:length(init_time)) = s_ocean;
    temp(1:xi_rho,1:eta_rho,1:N,1:length(init_time)) = 10;
  end
...
%   These values should be the same as in mod_param.F
    ...
    NPT = 1;   %number of inactive passive tracer   % AR
%
...
%            if i >= (xi_rho-2)                 % last two cell 
%              dye_temp(time,k,j,i) = dye_ini;
%            else
%              dye_temp(time,k,j,i) = 0;
%            end

            if (Gout.x_rho(j,i) >= xr_mouth+3e3)
%              dye_temp(time,k,j,i) = 0;
              dye_temp(j,i,k,time) = dye_ini;
            else
%              dye_temp(time,k,j,i) = 0;
              dye_temp(j,i,k,time) = dye_ini;
            end
...
% figure;
% xx = xr - xr(j_channel,i_mouth);
% subplot(2,1,1);
% contourf(repmat(xx(1,:),[N 1]),squeeze(z_r(:,j_channel,:)),squeeze(salt(1,:,j_channel,:)));
% xlim([-2e3 xx(j_channel,end)]); ylim([-60 0]); colorbar
% subplot(2,1,2);
% contourf(repmat(xx(1,:),[N 1]),squeeze(z_r(:,j_channel,:)),squeeze(dye_temp(1,:,j_channel,:)));
% xlim([-2e3 xx(j_channel,end)]); ylim([-60 0]); colorbar
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   bed_biodiff=ncread(data_file,'bed_biodiff',[1 1 1 tidx],[Inf Inf Inf 1]);
    ...
    bed_thickness(1:xi_rho,1:eta_rho,1:Nbed,1:length(init_time)) = 0.0;
